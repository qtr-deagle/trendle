...
class AdminUserService {
    ...
    public function getAllUsers($page = 1, $limit = 20, $search = '', $status = 'all') {
        ...
        try {
            ...
            // Get paginated users
            $query = 'SELECT id, username, email, display_name, avatar_url, bio, followers_count, 
                     following_count, is_active, is_admin, created_at, updated_at 
                     ' . $baseQuery . ' 
    ...
    public function getUserDetail($userId) {
        try {
            ...
            $stmt = $this->db->execute(
                'SELECT id, username, email, display_name, avatar_url, bio, followers_count, 
                        following_count, is_active, is_admin, created_at, updated_at 
                 FROM users WHERE id = ?',
                [$userId]
            );
